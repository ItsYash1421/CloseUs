...
export const UpdateRelationshipSheet: React.FC<UpdateRelationshipSheetProps> = ({
    visible,
    onClose,
}) => {
    ...
    const [livingStyle, setLivingStyle] = useState(user?.livingStyle || 'same_city');
    const [anniversary, setAnniversary] = useState<Date | null>(
        user?.anniversary ? new Date(user.anniversary) : null
    );
    const [expandedSection, setExpandedSection] = useState<'status' | 'living' | null>(null);
