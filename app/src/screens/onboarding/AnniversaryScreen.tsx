...
export const AnniversaryScreen = ({ navigation }: any) => {
    ...
    const existingAnniversary = storeData.anniversary ? new Date(storeData.anniversary) : null;
    const [day, setDay] = useState(existingAnniversary ? existingAnniversary.getDate().toString().padStart(2, '0') : '');
    const [month, setMonth] = useState(existingAnniversary ? (existingAnniversary.getMonth() + 1).toString().padStart(2, '0') : '');
    const [year, setYear] = useState(existingAnniversary ? existingAnniversary.getFullYear().toString() : '');
    ...
    const handleContinue = async () => {
        ...
        try {
            ...
            console.log('[AnniversaryScreen] Calling API: /api/users/complete-onboarding');
            const response: any = await apiClient.post('/api/users/complete-onboarding', payload);
            ...
            if (response.data && response.data.user) {
                console.log('[AnniversaryScreen] Updating user with API response:', response.data.user);
                console.log('[AnniversaryScreen] isOnboardingComplete in response:', response.data.user.isOnboardingComplete);
                ...
                setTimeout(() => {
                    ...
                    console.log('[AnniversaryScreen] isOnboardingComplete after update:', currentUser?.isOnboardingComplete);
                }, 100);
            } else {
                // Fallback: manually set onboarding complete
                console.log('[AnniversaryScreen] No user in response, using fallback');
                const currentUser = useAuthStore.getState().user;
                console.log('[AnniversaryScreen] Current user before fallback:', currentUser);
                if (currentUser) {
                    const updatedUser = {
                        ...currentUser,
                        isOnboardingComplete: true,
                    };
                    console.log('[AnniversaryScreen] Setting user with fallback:', updatedUser);
            ...
            console.log('[AnniversaryScreen] Final user state before navigation:', finalUser);
            console.log('[AnniversaryScreen] Final isOnboardingComplete:', finalUser?.isOnboardingComplete);
